...
var socials			= true;
var haveAsite		= false;
var globalpostslink	= "http://www.paginebaby.it"
...
$(function(){
  	//alert("bella "+getOsVersion());
  	$('#radius-field').val(radius+" Km");
	$("#add").hide();
	$("#loading").hide();
	mode = 1;
	if (jQT) setTimeout("nearMe(0)",3000);
    ...
    $("#address").blur(function(event, info) {
        updateMap();
        return false;       
    });
	$('#street_number').blur(function(event, info) {
        updateMap();
        return false;       
    });
	$('#zip').blur(function(event, info) {
        updateMap();
        return false;       
    });
	$('#city').blur(function(event, info) {
        updateMap();
        return false;       
    });
	$('#country').blur(function(event, info) {
        updateMap();
        return false;       
    });
});
...
function searchPlaces(skip,cc) {
    ...
    //if (str.length>1 || cat){
		...
		$.getJSON(url, {}, function(data){
			...
			$($('#search-results').find('.sep')[0]).html(data.msg);
			$($('#search-results').find('.more')[0]).remove();
			$('#search-results').append(placesSrc(data.data,"places")+(data.skip ? "<li class=\"more\"><a href=\"#\" onclick=\"searchPlaces("+data.skip+",'"+cat+"'); return false;\">"+getLabel("Visualizza altri risultati")+"</a></li>" : ""));
	    	$("#add").show();
			$("#loading").hide();
			if (jQT) jQT.setPageHeight();
		});
...
function loadFav(uid) {
	...
	$.getJSON(url, {}, function(data){
		if (data.length) {
			...
			$('#fav-results').html(placesSrc(favorities,"favorities"));
		} else {
			$('#fav-results').html("<li>"+getLabel("Nessun preferito")+"</li>");
		}
		if (jQT) jQT.setPageHeight();
	});
	_gaq.push(['_trackEvent', 'loadFav', url]);
}

function placesSrc(data,source) {
	str = "";
	if (data && data.length) {
		for (var i = 0; i < data.length; i++) {
			str+= '<li class="arrow'+(data[i]['hl']==1 ? ' hl' : '')+'">';
			str+= '<a href="#ipbdett" onclick="fillDett('+i+','+source+'); return false;">';
			if (data[i].img) {
...
function fillDett(i,source) {
	currentsource = eval(source);
	currentIndex = i;
	updateCurrentPosition();
	$('pid').val(currentsource[i]['id']);
	str = "<div class=\"dettCntMain\">";
	if (currentsource[i]['img']) str+= "<p class=\"imgCnt\"><img src=\""+currentsource[i]['img']+"\" /></p>";
	if (currentsource[i]['gallery']) str+= "<p><a class=\"galleryButton\" href=\"#photo\" onclick=\"generateGallery("+i+");return false;\">"+getLabel("Gallery")+"</a></p>";
	if (currentsource[i]['img'] && currentsource[i]['gallery']) str+= "<hr />";
	/*
	currentsource[i]['gallery'] = [
	...
	*/
	str+= "<h3>"+currentsource[i]['name']+"</h3>";
	str+= "<input type=\"hidden\" id=\"permalink\" value=\""+site_path+"/"+currentsource[i]['permalink']+"\" />";
	str+= "<input type=\"hidden\" id=\"author_loc\" value=\""+currentsource[i]['author']+"\" />";
	str+= "<p>"+currentsource[i]['address']+(currentsource[i]['street_number'] ? " "+currentsource[i]['street_number'] : "")+", "+currentsource[i]['zip']+" - "+currentsource[i]['city']+"</p>";
	if (currentsource[i]['pbcard']==1) str+= "<p><span class=\"pbcListCnt\">PB-CARD</span></p>";
	if (currentsource[i].distance) str+= "<p><span class=\"stitTit\">"+getLabel("Distanza")+": </span> "+Math.floor(currentsource[i]['distance']*100)/100+" km</p>";
	...
	str+= "<p><span class=\"stitTit\">"+getLabel("Checkins")+": </span> "+currentsource[i]['checkin']+"</p>";
	if (currentsource[i]['tel']) str+= "<p><span class=\"stitTit\">"+getLabel("Tel")+": </span> <a href=\"tel:"+currentsource[i]['tel']+"\">"+currentsource[i]['tel']+"</a></p>";
	if (currentsource[i]['fax']) str+= "<p><span class=\"stitTit\">"+getLabel("Fax")+": </span> "+currentsource[i]['fax']+"</p>";
	if (currentsource[i]['email']) str+= "<p><span class=\"stitTit\">"+getLabel("Email")+": </span> <a href=\"mailto:"+currentsource[i]['email']+"\">"+currentsource[i]['email']+"</a></p>";
	if (currentsource[i]['website']) str+= "<p class=\"website\"><span class=\"stitTit\">"+getLabel("Sito web")+": </span> <a href=\""+currentsource[i]['website']+"\" target=\"_blank\">"+currentsource[i]['website']+"</a></p>";
	if (currentsource[i]['cat']) str+= "<p class=\"catcat\"><span class=\"stitTit\">"+getLabel("Categorie")+": </span> "+currentsource[i]['cat_str']+"</p>";
	/*str+= "<p>categories:</p>";
	for (var a = 0; a < currentsource[i].categories.length; a++) {
		str+= "<p>"+currentsource[i].categories[a].name+"</p>";
	}*/
	str+= "<hr />";
	str+= "<div class=\"txt\">"+currentsource[i]['txtHtml']+"<br class=\"myClear\" /></div>";
	//str+= "<p>tipCount: "+currentsource[i]['stats']['tipCount']+"</p>";
	//str+= "<p>usersCount: "+currentsource[i]['stats']['usersCount']+"</p>";
	str+= "<hr />";
	str+= "<div id=\"mappaDett\"><img alt=\""+getLabel("Mappa di")+": "+currentsource[i]['name']+"\" src=\"http://maps.googleapis.com/maps/api/staticmap?zoom=14&size=280x200&maptype=roadmap&markers=color:red|color:red|label:S|"+currentsource[i]['lat']+","+currentsource[i]['lng']+"&sensor=false\" /></div>";
	str+= "<hr />";
	/*
	currentsource[i]['gallery'] = [
		  {src:"http://www.nasa.gov/images/content/450090main_image_1653_946-710.jpg",width: 946,height: 710},
	...
	if (jQT) jQT.generateGallery("photo", currentsource[i]['gallery'],{defaultIndex:0});
	str+= "<hr />";
	*/
	str+= "<ul class=\"individual\">";
	//console.log(fava);
		str+= "<li><a class=\"greenButton\" rel=\"external\" target=\"_blank\" href=\"#\" onclick=\"#\"= id=\"openmaps\">"+getLabel("ARRIVA QUI")+"</a></li>";
		str+= "<li><a class=\"greenButton\" onclick=\"$('#pid2').val("+currentsource[i]['id']+");\" href=\"#propietario\">"+getLabel("SEGNALA")+"</a></li>";

	if (fava && $.inArray(currentsource[i]['id'],fava)!=-1) {
		str+= "<li><a id=\"favoritiesEdit\" class=\"greenButton\" onclick=\"favoritiesEdit("+currentsource[i]['id']+");\" href=\"#\">"+getLabel("RIMUOVI DAI PREFERITI")+"</a></li>";
	} else {
		str+= "<li><a id=\"favoritiesEdit\" class=\"greenButton\" onclick=\"favoritiesEdit("+currentsource[i]['id']+");\" href=\"#\">"+getLabel("AGGIUNGI AI PREFERITI")+"</a></li>";
	}
	str+= "<li><a class=\"greenButton\" onclick=\"checkin("+currentsource[i]['id']+");\" href=\"#\">"+getLabel("CHECK-IN")+"</a></li>";
	//str+= "</ul>";
	//str+= "<ul class=\"individual\">";
	str+= "<li><a class=\"greenButton\" onclick=\"rate("+currentsource[i]['id']+");\" href=\"#rate\">"+getLabel("VOTA")+"</a></li>";
	//console.log($('#uid').val());
	if (currentsource[i]['author']==$('#uid').val()) {
		str+= "<li><a class=\"greenButton\" onclick=\"onEdit("+i+");\" href=\"#aggiungi\">"+getLabel("MODIFICA")+"</a></li>";
	} else {
		str+= "<li><a class=\"greenButton\" onclick=\"$('#pid2').val("+currentsource[i]['id']+");\" href=\"#propietario\">"+getLabel("SEI IL TITOLARE")+"?</a></li>";
	}
	str+= "</ul>";
	str+= "</div>";
	if (jQT) {
	...
	$('.rate_widget').each(function(ii) {
        $(this).find('.star_' + parseInt(currentsource[i]['rate'])).prevAll().andSelf().addClass('ratings_vote');
        $(this).find('.star_' + parseInt(currentsource[i]['rate'])).nextAll().removeClass('ratings_vote'); 
    });
	_gaq.push(['_trackPageview', site_path+"/"+currentsource[i]['permalink']]);
}
...
function onEdit(i) {
	...
	*/
	_gaq.push(['_trackEvent', 'Edit', places[i]['id']+ " " + places[i]['name']]);
}
...
function setRadius() {
	...
	_gaq.push(['_trackEvent', 'setRadius', radius]);
}
	
function onAdd() {
	if ($('#author') && $('#author').val() && $('#author').val()!="") {
		if (mode) {
			updateCurrentPosition();
		}
...
function updateMap(){
	address = $("#address").val()+", "+$("#street_number").val()+", "+$("#zip").val()+", "+$("#city").val()+", "+$("#country").val();
    //console.log(address);
    //console.log(geocoder);
	if (!geocoder) geocoder = new google.maps.Geocoder();
    geocoder.geocode( { 'address': address}, function(results, status) {
		llat = results[0].geometry.location.lat();
		llng = results[0].geometry.location.lng();
	   	$("#ll").val(llat + ',' + llng);
	...
	_gaq.push(['_trackEvent', 'EditUpdateMap', address]);
}
...
function editPosMap() {
	...
	_gaq.push(['_trackEvent', 'editPosMap', address]);
}
		...
		if(data.results.length) {
			...
			if (currentIndex) {
				if (getOsVersion()<6) {
					...
					saddr = "http://maps.apple.com/maps?saddr="+escape($('#address').val()+($('#street_number').val() ? ", "+$('#street_number').val() : "")+", "+$('#zip').val()+", "+$('#city').val()+", "+$('#country').val());
				}
				//alert(saddr);
				$("#openmaps").attr('href',saddr);
				//$("#openmaps").attr('onclick',"alert('"+saddr+"'");
			}
		}
	});
	_gaq.push(['_trackPageview', url]);
...
function placeSave() {
	...
	if (chk) {
		...
		$.post(url, obj, function(data, textStatus) {
			...
			$('#search-results').html("<li class=\"sep\">"+getLabel("Risultati")+"</li>"+placesSrc(places,"places"));
			if (jQT) jQT.setPageHeight();
			$('#pid').val(data);
			if (jQT) jQT.goBack();
			if (socials) {
				if(confirm(getLabel("Grazie per il nuovo inserimento")+"!!!\n\n"+getLabel("Vuoi condividere su Facebook")+"?")) {
					...
					params.link = (haveAsite ? $("#permalink").val() : globalpostslink);
		...
		_gaq.push(['_trackPageview', url+"/"+obj.stringify()]);
		return true;
	} else {
		return false;
	}
...
function sendMail(form) {
	...
	if (chk) {
		...
		_gaq.push(['_trackEvent', 'sendMail', response]);
		return true;
	} else {
		return false;
	}
...
function favoritiesEdit(pid) {
	if ($('#uid').val()) {
		...
		$.getJSON(url, {}, function(data){
			...
			//console.log(data);
			if (data.err){
				$("#favoritiesEdit").html(getLabel("AGGIUNGI AI PREFERITI"));
				alert(getLabel("Rimosso dai preferiti")+"!!!");
				$("#loading").hide();
			} else {
				$("#favoritiesEdit").html(getLabel("RIMUOVI DAI PREFERITI"));
				if (socials) {
...
function postToFacebook(params) {
	$.ajax({
		type: "POST",
		url: 'https://graph.facebook.com/'+facebookFP+'/feed',
		dataType: "jsonp",
		data:params,
		success: function(data) {
			...
			$.ajax({
				type: "POST",
				url: 'https://graph.facebook.com/'+$("#uid").val()+'/feed',
				dataType: "jsonp",
				data:params,
				success: function(data) {
					//console.log(data);
					if ($("#uid").val()!=$("#author_loc").val()) {
						$.ajax({
							type: "POST",
							url: 'https://graph.facebook.com/'+$("#author_loc").val()+'/feed',
							dataType: "jsonp",
							data:params,
							success: function(data) {
								//console.log(data);
	...
	_gaq.push(['_trackEvent', 'FacebookShare', params]);
}
...
function loginLog(response) {
	...
	_gaq.push(['_trackEvent', 'Login', response]);
}
...
function loadFBpage() {
	//console.log("loadFBpage");
	//console.log('https://graph.facebook.com/'+facebookFP+'/feed?access_token='+at);
	$.getJSON('https://graph.facebook.com/'+facebookFP+'/feed?access_token='+at, {}, function(response){
